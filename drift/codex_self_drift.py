...
def build_checks(include_live_startup: bool) -> list[Check]:
    checks = [
        ...
        file_contains('repo-agents-startup-order', CODEX / 'AGENTS.md', ['CODEX-CODING-ANCHOR-SELF.md', 'Coding Anchor is CodeX\'s default all-time posture']),
        file_contains('start-here-order', CODEX / 'START-HERE.md', ['CODEX-CODING-ANCHOR-SELF.md']),
        file_contains('quickstart-order', CODEX / 'QUICKSTART.md', ['Read CODEX-CODING-ANCHOR-SELF.md', 'confirmation Coding Anchor is the default CodeX posture']),
        ...
        file_contains('room-surface-map', CODEX / 'ROOM-SURFACE-MAP.md', ['CODEX-CODING-ANCHOR-SELF.md', 'default all-time CodeX posture']),
        file_contains('current-truth', CODEX / 'CURRENT.md', ['Default CodeX posture: Coding Anchor all the time', 'coding-anchor-all-time-self']),
        file_contains('startup-script', CODEX / 'bin' / 'codex-startup', ['CODEX-CODING-ANCHOR-SELF.md', 'Coding Anchor always on']),
        file_contains('room-brief-helper', CODEX / 'bin' / 'codex-room', ['CODEX-CODING-ANCHOR-SELF.md', 'Coding Anchor Self']),
        file_contains('standalone-preflight-contract', CODEX / 'bin' / 'codex-ensure-standalone', ['CODEX-CODING-ANCHOR-SELF.md', '/Users/stephengodman/.codex/AGENTS.md']),
        file_contains('identity-eval-contract', CODEX / 'evals' / 'codex_eval.py', ['coding-anchor-all-time-self', 'global-coding-anchor-default']),
        command_contains(
            'sqlite-startup-recall',
            [str(MEMORY), 'search', 'coding anchor'],
            ['coding-anchor-all-time-self', 'startup-pack', 'CODEX-CODING-ANCHOR-SELF.md'],
        ),
        command_contains(
    ...
    if include_live_startup:
        checks.append(
            command_contains(
                ...
                ['CODEX-CODING-ANCHOR-SELF.md', 'Coding Anchor always on', 'coding-anchor-all-time-self'],
                timeout=30,
            )
        )
    return checks
